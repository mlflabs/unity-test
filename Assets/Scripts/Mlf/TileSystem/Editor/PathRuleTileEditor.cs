...
{
    [CustomEditor(typeof(PathRuleTile), true)]
    [CanEditMultipleObjects]
    public class PathRuleTileEditor : Editor
    {
        ...
        public PathRuleTile tile => target as PathRuleTile;
        public ReorderableList m_ReorderableList;
        public bool extendNeighbor;

        public PreviewRenderUtility m_PreviewUtility;
        public UnityEngine.Grid m_PreviewGrid;
        public List<UnityEngine.Tilemaps.Tilemap> m_PreviewTilemaps;
        public List<UnityEngine.Tilemaps.TilemapRenderer> m_PreviewTilemapRenderers;
        ...
        {
            m_ReorderableList = new ReorderableList(tile.m_TilingRules, typeof(PathRuleTile.PathTilingRule), true, true, true, true);
        ...
        public virtual BoundsInt GetRuleGUIBounds(BoundsInt bounds, PathRuleTile.PathTilingRule rule)
        {
            if (extendNeighbor)
            {
                bounds.xMin--;
        ...
        {
            PathRuleTile.PathTilingRule rule = tile.m_TilingRules[index];
            return GetElementHeight(rule);
        }

        public float GetElementHeight(PathRuleTile.PathTilingRule rule)
        {
            BoundsInt bounds = GetRuleGUIBounds(rule.GetBounds(), rule);

            float inspectorHeight = GetElementHeight(rule as PathRuleTile.TilingRuleOutput);
            float matrixHeight = GetMatrixSize(bounds).y + 10f;

            return Mathf.Max(inspectorHeight, matrixHeight);
        }

        public float GetElementHeight(PathRuleTile.TilingRuleOutput rule)
        {
            float inspectorHeight = k_DefaultElementHeight + k_PaddingBetweenRules;

            switch (rule.m_Output)
            {
                case PathRuleTile.PathTilingRule.OutputSprite.Random:
                    inspectorHeight = k_DefaultElementHeight + k_SingleLineHeight * (rule.m_Sprites.Length + 3) + k_PaddingBetweenRules;
                    break;
                case PathRuleTile.PathTilingRule.OutputSprite.Animation:
        ...
        {
            PathRuleTile.PathTilingRule rule = new PathRuleTile.PathTilingRule();
            rule.m_Output = PathRuleTile.PathTilingRule.OutputSprite.Single;
        ...
        public static void UpdateAffectedOverrideTiles(PathRuleTile target)
        {
            List<PathRuleOverrideTile> overrideTiles = FindAffectedOverrideTiles(target);
        ...
        public static List<PathRuleOverrideTile> FindAffectedOverrideTiles(PathRuleTile target)
        {
            List<PathRuleOverrideTile> overrideTiles = new List<PathRuleOverrideTile>();

            string[] overrideTileGuids = AssetDatabase.FindAssets("t:" + typeof(PathRuleOverrideTile).Name);
            foreach (string overrideTileGuid in overrideTileGuids)
            {
                ...
                PathRuleOverrideTile overrideTile = AssetDatabase.LoadAssetAtPath<PathRuleOverrideTile>(overrideTilePath);
        ...
        public virtual void RuleOnGUI(Rect rect, Vector3Int position, int neighbor)
        {
            switch (neighbor)
            {
                case PathRuleTile.PathTilingRule.Neighbor.This:
                    GUI.DrawTexture(rect, arrows[GetArrowIndex(position)]);
                    break;
                case PathRuleTile.PathTilingRule.Neighbor.NotThis:
        ...
        public virtual void RuleTransformOnGUI(Rect rect, PathRuleTile.PathTilingRule.Transform ruleTransform)
        {
            switch (ruleTransform)
            {
                case PathRuleTile.PathTilingRule.Transform.Rotated:
                    GUI.DrawTexture(rect, autoTransforms[0]);
                    break;
                case PathRuleTile.PathTilingRule.Transform.MirrorX:
                    GUI.DrawTexture(rect, autoTransforms[1]);
                    break;
                case PathRuleTile.PathTilingRule.Transform.MirrorY:
                    GUI.DrawTexture(rect, autoTransforms[2]);
                    break;
                case PathRuleTile.PathTilingRule.Transform.Fixed:
                    GUI.DrawTexture(rect, autoTransforms[3]);
                    break;
                case PathRuleTile.PathTilingRule.Transform.MirrorXY:
                    GUI.DrawTexture(rect, autoTransforms[4]);
                    break;
            }
        }

        public void RuleNeighborUpdate(Rect rect, PathRuleTile.PathTilingRule tilingRule, Dictionary<Vector3Int, int> neighbors, Vector3Int position)
        {
        ...
        public void RuleTransformUpdate(Rect rect, PathRuleTile.PathTilingRule tilingRule)
        {
            if (Event.current.type == EventType.MouseDown && ContainsMousePosition(rect))
            {
                tilingRule.m_RuleTransform = (PathRuleTile.PathTilingRule.Transform)(int)Mathf.Repeat((int)tilingRule.m_RuleTransform + GetMouseChange(), Enum.GetValues(typeof(PathRuleTile.PathTilingRule.Transform)).Length);
                GUI.changed = true;
                Event.current.Use();
            }
        }
        ...
        public virtual void RuleMatrixOnGUI(PathRuleTile tile, Rect rect, BoundsInt bounds, PathRuleTile.PathTilingRule tilingRule)
        {
        ...
        public void RuleMatrixIconOnGUI(PathRuleTile.PathTilingRule tilingRule, Dictionary<Vector3Int, int> neighbors, Vector3Int pos, Rect rect)
        {
        ...
        public virtual void SpriteOnGUI(Rect rect, PathRuleTile.TilingRuleOutput tilingRule)
        {
            tilingRule.m_Sprites[0] = EditorGUI.ObjectField(rect, tilingRule.m_Sprites[0], typeof(Sprite), false) as Sprite;
        }

        public void RuleInspectorOnGUI(Rect rect, PathRuleTile.TilingRuleOutput tilingRule)
        {
            float y = rect.yMin;
            ...
            tilingRule.m_Output = (PathRuleTile.PathTilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
            y += k_SingleLineHeight;

            if (tilingRule.m_Output == PathRuleTile.PathTilingRule.OutputSprite.Animation)
            {
            ...
            if (tilingRule.m_Output == PathRuleTile.PathTilingRule.OutputSprite.Random)
            {
                ...
                tilingRule.m_RandomTransform = (PathRuleTile.PathTilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RandomTransform);
                y += k_SingleLineHeight;
            }

            if (tilingRule.m_Output != PathRuleTile.PathTilingRule.OutputSprite.Single)
            {
        ...
        public virtual void CreatePreview()
        {
            ...
            m_PreviewGrid = previewInstance.AddComponent<UnityEngine.Grid>();
            m_PreviewUtility.AddSingleGO(previewInstance);

            m_PreviewTilemaps = new List<UnityEngine.Tilemaps.Tilemap>();
            m_PreviewTilemapRenderers = new List<UnityEngine.Tilemaps.TilemapRenderer>();

            for (int i = 0; i < 4; i++)
            {
                var previewTilemapGo = new GameObject();
                m_PreviewTilemaps.Add(previewTilemapGo.AddComponent<UnityEngine.Tilemaps.Tilemap>());
                m_PreviewTilemapRenderers.Add(previewTilemapGo.AddComponent<UnityEngine.Tilemaps.TilemapRenderer>());
        ...
        [Serializable]
        class PathRuleTileRuleWrapper
        {
            [SerializeField]
            public List<PathRuleTile.PathTilingRule> rules = new List<PathRuleTile.PathTilingRule>();
        }

        [MenuItem("CONTEXT/PathRuleTile/Copy All Rules")]
        public static void CopyAllRules(MenuCommand item)
        {
            PathRuleTile tile = item.context as PathRuleTile;
            if (tile == null)
                return;

            PathRuleTileRuleWrapper rulesWrapper = new PathRuleTileRuleWrapper();
        ...
        [MenuItem("CONTEXT/PathRuleTile/Paste Rules")]
